...
def getFilteredDataOld(imattra, imdata, power, sign):
    #print(imattra)
    print(imattra.shape,imdata.shape,power,sign)
    ...
    for i in range(imdata.shape[0]):
        for j in range(imdata.shape[1]):
            ...
            x2 = cd2**power;
            for l in range(samples):
                filterFlag = randomFilter(x0,x1,x2)  
    ...
    tot = imdata.shape[0]*imdata.shape[1]*samples
    print("count is",count, " vs ", tot, " percent ",count*100/tot )
    return filteredDataNp
...
origFlag = int(sys.argv[5])
targetLabel = int(sys.argv[6])
...
for i in range(5):
    ...
    for j in range(0,batch_size):
        ...
        #targeted attack is successfull only when the new label is equal to the target label
        if topLabel == labels[j]:
            counterSame += 1
            sameFlag[count] = 1
        elif topLabel == targetLabel:
            counterPert += 1
        else:
            sameFlag[count] = 1
        ...
        for k in range(len(power)):
            if sameFlag[count] == 0: #only calculate when attack has been successful
                ...
                print("crossEntropy ",crossEntropy," entropy",entropy," flip",flipProbStore[k][count])
